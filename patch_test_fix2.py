...
# Since run_dbg.py runs fine, but tests/transport/test_step5_transport.py fails with segfault,
# the reason must be related to `unittest` capturing something or destroying objects
# that Numba tries to read.
# Maybe we can wrap `TestStep5TransportLogic` to just be a script.
