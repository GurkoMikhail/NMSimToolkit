...
class MaterialsDataBase:
    ...
    pathToMaterialsTable: str = 'tables/materials.hdf'
    pathToTableOfAC: str = 'tables/attenuationCoefficients.hdf'
    baseName: str = 'NIST'
    ...
    def materialsNamesList(self):
        ...
        for materialTypeGroup in baseGroup.values():
            for materialName in materialTypeGroup:
    ...
    def materialsList(self):
        """ Список материалов """
        return [Material.load(name, self) for name in self.materialsNamesList]

    @cache
    def getMaterialByName(self, materialName):
        """ Получить материал """
        return Material.load(materialName, self)
    ...
    def getMaterialByID(self, materialID):
        ...
        for material in self.materialsList:
            if material.ID == materialID:
                return material
        raise NameError(f'Материал с ID {materialID} не найден')

    @cache
    def __getitem__(self, material):
        if isinstance(material, str):
            return self.getMaterialByName(material)
        if isinstance(material, int):
            return self.getMaterialByID(material)
        raise KeyError
...
class Material:
    ...
    @classmethod
    @cache
    def load(cls, name, materialsDataBase=MaterialsDataBase()):
        materialsFile = File(materialsDataBase.pathToMaterialsTable, 'r')
        baseGroup = materialsFile[materialsDataBase.baseName]
            ...
            for materialName, materialGroup in materialTypeGroup.items():
                if name == materialName:
                    ...
                    try:
                        ...
                        print(f'Для {name} отсутствует Z\\A')
                        ZtoAratio = 0.5
                    ID = next(materialsDataBase.counter)
                    return cls(name, type, density, composition, ZtoAratio, ID)
