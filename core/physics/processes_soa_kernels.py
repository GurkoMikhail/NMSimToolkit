...
def _push_to_interaction_buffer(
    ...
    inter_buffer.volume_id[idx] = volume_id
    inter_buffer.material_id[idx] = material_id
...
def make_photoelectric_kernel(process_id: ProcessID):
    ...
    @njit(cache=True, inline='always')
    def _photoelectric_device_func(p_idx: Index, state: KinematicState, particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer) -> None:
        energy_deposit = state.energy[p_idx]
        state.energy[p_idx] = 0.0
        _push_to_interaction_buffer(
            inter_buffer, process_id_c, particle_ids[p_idx], energy_deposit, 0.0, 0.0, current_volumes[p_idx], material_ids[p_idx],
    ...
    @njit(cache=True)
    def _photoelectric_kernel(state: KinematicState, target_indices: NDArray[Index], particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer, physics_buffer: PhysicsBuffer, rng_ctx: RNGContext) -> None:
...
def make_compton_kernel(process_id: ProcessID):
    process_id_c = ProcessID(process_id)
    @njit(cache=True, inline='always')
    def _compton_device_func(p_idx: Index, state: KinematicState, particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer, physics_buffer: PhysicsBuffer, rng_ctx: RNGContext) -> None:
        ...
        _push_to_interaction_buffer(
            inter_buffer, process_id_c, particle_ids[p_idx], energy_deposit, theta, phi, current_volumes[p_idx], mat_id,
            state.position.x[p_idx], state.position.y[p_idx], state.position.z[p_idx],
            state.direction.x[p_idx], state.direction.y[p_idx], state.direction.z[p_idx]
        )
    @njit(cache=True)
    def _compton_kernel(state: KinematicState, target_indices: NDArray[Index], particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer, physics_buffer: PhysicsBuffer, rng_ctx: RNGContext) -> None:
        for j in range(len(target_indices)):
            p_idx = target_indices[j]
...
def make_coherent_kernel(process_id: ProcessID):
    process_id_c = ProcessID(process_id)
    @njit(cache=True, inline='always')
    def _coherent_device_func(p_idx: Index, state: KinematicState, particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer, physics_buffer: PhysicsBuffer, rng_ctx: RNGContext) -> None:
        energy = state.energy[p_idx]
        mat_id = material_ids[p_idx]
        Z = _sample_Z_scalar(mat_id, physics_buffer, rng_ctx)
    ...
    @njit(cache=True)
    def _coherent_kernel(state: KinematicState, target_indices: NDArray[Index], particle_ids: NDArray[ID], current_volumes: NDArray[Index], material_ids: NDArray[Index], inter_buffer: InteractionBuffer, physics_buffer: PhysicsBuffer, rng_ctx: RNGContext) -> None:
        for j in range(len(target_indices)):
            p_idx = target_indices[j]
